...
Matrix set := method(dataList,
    ...
    self x = data size
    if(self x > 0, self y = data at(0) size, self y = 0)
    self
)

Matrix print := method(
    data foreach(row,
        row foreach(element, element asString alignLeft(4, " ") print)
        "" println
    )
)

Matrix multiply := method(other,
    if(self y != other x, Exception raise("Matrix dimensions incompatible for multiplication."))
